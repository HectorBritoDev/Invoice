            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        ...
                        <button class="btn btn-success" data-target="#createBudgetModal" data-toggle="modal">Agregar</button>
                ...
                <div class="card-body ">
                    <table class="table table-hover" id="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $budget->serie }} - {{ $budget->code }}</td>
                                <td>{{ $budget->client_name }}</td>
                                <td>{{ $budget->emission_date }}</td>
                                <td>{{ $budget->expiration_date }}</td>
                                <td>{{ $budget->items()->sum('total') }}</td>
                                <td>{{ $budget->status }}</td>
                                <td>


                                    {{-- <form id="deleteBudgetForm" class="form-inline" action="{{ route('budget.destroy',$budget) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <i class="fa fa-trash red" onclick="deleteBudget()">
                                        </form>
                                        / --}}
                                        <a href="{{ route('budget.edit',$budget) }}"><i class="fas fa-eye"></i></a>
                ...
                <div class="modal-header">
                    <div class="input-group">
                        ...
                        <div class="form-group col-md-5">
                            <input type="text" name="serie" id="serie" class="form-control" @if ($lastBudget !=null)
                                value="{{ old('serie',$lastBudget->serie) }}" @endif placeholder="Codigo">
                            @if ($errors->has('serie'))
                        ...
                        <div class="form-group col-md-5">
                            <input type="number" name="code" id="code" class="form-control" placeholder="Empieza desde"
                                @if ($lastBudget !=null) value="{{ old('code',$lastBudget->code + 1) }}" @endif>
                            @if ($errors->has('code'))
                            <span style="color:red;">
                                {{ $errors->first('code') }}
                            </span>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="form-group col-md-3">
                            <select name="coin" id="coin" class="form-control select2">
                                {{-- @if ($budget->coin != 'Soles' && $budget->coin != 'Dolares' && $budget->coin !=
                                    null)
                                    <option value="{{ $budget->coin }}"
                                selected>{{ $budget->coin }}</option>
                                @else
